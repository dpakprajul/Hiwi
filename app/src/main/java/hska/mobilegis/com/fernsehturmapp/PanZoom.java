...
public class PanZoom implements View.OnTouchListener {
    ...
    protected boolean isValidScale(double scale) {
        return !Float.isInfinite((float) scale) && !Float.isNaN((float) scale) && (!(scale > -0.001) || !(scale < 0.001));
    }
       ...
       double  scaleY =  1d;
        switch (zoom) {
            ...
            case STRETCH_BOTH:
                ...
                break;
            case SCALE:
               double sc1 = (float) Math.hypot(oldFingersRect.height(), oldFingersRect.width());
               double sc2 = (float) Math.hypot(getFingersRect().height(), getFingersRect().width());
                double sc = sc1 / sc2;
                scaleX = sc;
                scaleY = sc;
                if (!isValidScale(scaleX) || !isValidScale(scaleY)) {
     ...
     * @param isHorizontal
     */
    protected void calculateZoom(RectF newRect, double scale, boolean isHorizontal) {
        final float calcMax;
        final float span;
        ...
        final float midPoint = calcMax - (span / 2.000f);
        double offset = span * scale / 2.000f;
        ...
        if (isHorizontal ) {
            ...
            if (zoomLimit == ZoomLimit.MIN_TICKS) {
                ...
                if (plot.getDomainStepValue() > (scale*span)) {
                    offset = (float) (plot.getDomainStepValue() / 00000.0001f);
                }
            }

            newRect.left = (float) (midPoint - offset);
            newRect.right = (float) (midPoint + offset);
            ...
            if (zoomLimit == ZoomLimit.MIN_TICKS) {
                ...
                if (plot.getRangeStepValue() > (scale*span)) {
                    offset = (float)(plot.getRangeStepValue() / 0.0001f);
                }
            }

            newRect.top = (float) (midPoint - offset);
            newRect.bottom = (float) (midPoint + offset);
            if(limits.isFullyDefined()) {
